    ...
    <div class="body">
          ...
          <div class="flex">
            ...
            <div class="flex-1">
              <div class="select-category flex h-center">
                <div class="flex-1">
                  <div class="title flont-18 gray bold">选择一级分类</div>
          ...
          <div class="flex">
            ...
            >
              <el-form-item label="商品分类：">
                <span class="font-14 green bold" @click="stepActive = 1">{{ categoryName }}</span>
              ...
              <el-form-item label="商品货号：" prop="goodsNo">
                ...
                <p class="form-tips">如果你不输入商品货号，系统将自动生成一个唯一的货号</p>
              </el-form-item>
              ...
              <el-form-item label="商品库存：" prop="goodsStock">
                <el-input v-model="ruleForm.goodsStock" type="bnumber"></el-input>
                <p class="form-tips">该设置只对单品有效，当商品存在多规格货品时为不可编辑状态，库存数值取决于货品数量</p>
              </el-form-item>
              <el-form-item label="库存预警值：" prop="goodsWarning">
                <el-input v-model="ruleForm.goodsWarning"></el-input>
              </el-form-item>
              ...
              <el-form-item>
                <el-button @click="stepActive = 1">返回上一步,选择商品分类</el-button>
                  ...
                  <div class="prop-wrap">
                    <el-checkbox-group v-model="checkProp" v-for="(item,index) in propList" :key="index">
                      ...
                      <el-checkbox
                        ...
                        v-for="(prop,index) in item.value"
                        :key="index"
                      >{{ prop }}</el-checkbox>
                    </el-checkbox-group>
                    <div>
              ...
              <table class="goods-table" border="1" v-show="propSpecList.length>0">
                ...
                <tbody>
                  <tr v-for="(item,index) in propSpecList" :key="index">
                    ...
                    <td>
                      <el-input size="mini" type="number" :max="99999" v-model="item.goodsSalePrice"></el-input>
                    </td>
                    <td>
                      <el-input size="mini" type="number" :max="99999" v-model="item.goodsStock"></el-input>
                    </td>
                    <td>
                      <el-input size="mini" type="number" :max="99999" v-model="item.stockWarning"></el-input>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="flex-1">
              ...
              <div class="flex h-center">
                <div class="upload-btn-wrap">
                  ...
                  <input type="file" id="upload" class="upload-input large" accept="image/jpeg,image/png" @change="uploadGoodImg">
                </div>
                ...
                <div class="flex-1">
                  ...
                  <ul class="category-list">
                    ...
                    >
                      ...
                      <i class="el-icon-arrow-right hf"></i>
                    </li>
                  </ul>
                </div>
                <img src="@/assets/icon-arrow.png" alt="">
                <div class="flex-1">
                  ...
                  <ul class="category-list" v-if="childHeaderList.length > 0">
                    <li
                      ...
                      v-for="(item,index) in childHeaderList"
                      :key="index"
                    >{{ item.navName }}</li>
                  </ul>
      ...
      <el-dialog title="从图库选择" :visible.sync="dialogVisible" :append-to-body="true">
        <div class="flex between h-center">
          <span>商品图库 > 全部图片</span>
          <el-select v-model="input" placeholder="请选择图库分类" size="small" class="search-input" @change="getAlbumImg">
        ...
        <div class="img-table flex">
          <div v-for="(item,index) in albumImgList" :key="index" @click="checkAlbumImg(item)">
            <!-- indexof在checkAlbumImgList中查找item.imgUrl，找不到的话则返回-1 -->
            <img :src="item.imgUrl" alt="">
            <i class="el-icon-success" v-if="checkAlbumImgList.indexOf(item.imgUrl) >= 0"></i>
          </div>
        </div>
        <pagination :isBatch="false"
          @next="next"
          :total="total"
          :pageSize="pageSize"
          class="pagination"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      editorOption: {
        // 自定义富文本
        placeholder: "在这里输入内容",// 提示语
        theme: "snow", // or 'bubble'定义主题
        modules: {
          toolbar: {
            ...
            handlers: {
              ...
              image: function(value) {
                // 若是上传图片，则将el-upload节点触发，上传至服务器；如不上传图片，则将图片置空
                if (value) {
            ...
            .then(res => {
              ...
              if (this.$route.query.id) {
                ...
                .then(res => {
                  ...
                  // this.$set添加响应式的新属性，（添加到哪个对象，属性名，属性值)
                  ...
                  this.$set(this.ruleForm,'goodsImg',res.goodsImg)
  ...
  methods:{
    ...
    handleCheckProp(prop,name,$event){
      if ($event) {
        ...
        for (let i = 0; i < this.checkPropList.length; i++) {
          if (this.checkPropList[i].name == name) {
            for (let j = 0; j < this.checkPropList[i].length; j++) {
    ...
    uploadGoodImg(file){
      ...
      for (let i = 0; i < files.length; i++) {
        if (files[i] == []) {
          continue
          // 语句中断（循环中）的一个迭代，跳到下一个循环
        }
        // uploadFiles函数在mixin内定义，将图片上传到服务器，并且定义返回图片在服务器的位置imgUrl
        promiseList.push(this.uploadFiles(files[i]))
      }
      // Promise是一个异步，resolve为正确回调，reject为失败回调。promise.all方法用于将多个 Promise 实例，包装成一个新的Promise实例。
    ...
    checkAlbumImg(data){
      if (this.checkAlbumImgList.indexOf(data.imgUrl) < 0) {
        ...
        this.checkAlbumImgList.splice(
          this.checkAlbumImgList.indexOf(data.imgUrl),1
        )
      }
    },
    next(val){
    ...
    submitGoods() {
      ...
      // JSON.parse(JSON.stringify(obj))用来深拷贝,利用JSON.stringify 将js对象序列化（JSON字符串），再使用JSON.parse来反序列化(还原)js对象；序列化的作用是存储和传输
